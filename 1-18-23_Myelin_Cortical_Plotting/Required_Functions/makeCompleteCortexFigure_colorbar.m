function makeCompleteCortexFigure_colorbar(outfile,AllSurfs,AllSurfFaces,AllIndices2Plot,AllFaces2Plot,...
...
% colorbar tick label formatting
numTicks = 5; % Change this value to the number of ticks you want
minVal   = min(cmap.caxis);
maxVal   = max(cmap.caxis);
cticks   = minVal:(maxVal-minVal)/(numTicks-1):maxVal;
R        = 16; % rounding value: 2 for nearest 0.5, 4 for nearest 0.25
small    = 0;

%%% if both max and min are smaller than 1/R, use sig figs instead
if abs(max(cmap.caxis)) < 1/R && abs(min(cmap.caxis)) < 1/R
    minLabel = round(min(cmap.caxis)*R,2,'significant')/R;
    maxLabel = round(max(cmap.caxis)*R,2,'significant')/R;
    small    = 1;
else
    minLabel = round(min(cmap.caxis)*R)/R;
    maxLabel = round(max(cmap.caxis)*R)/R;
end
nLabels  = minLabel:(maxLabel-minLabel)/(numTicks-1):maxLabel;
if ~small, 
    formSpec = '%.2f';
%     formSpec = '%.2e'; % for "e" notation
else
    formSpec = '%.1e';
end
for i=1:length(nLabels)
   labels{i} = num2str(nLabels(i),formSpec);
end
cb = colorbar('Ticks',cticks,'TickLabels',labels);

% colorbar size and position formatting
%set(cb,'position',[.916 .325 .025 .375]);
set(cb,'position',[.908 .325 .025 .375]);

l=get(cb,'YTickLabel');
% set(cb,'YTickLabel',l,'fontsize',6);
set(cb,'YTickLabel',l,'fontsize',5);
