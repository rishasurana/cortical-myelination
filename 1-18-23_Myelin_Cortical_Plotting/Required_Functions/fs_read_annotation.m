...
if(isempty(bool)) %means no colortable
  fprintf('%s: ERROR: File %s does not contain a color table.\n',mfilename,fname);
  colortable = struct([]);
  fclose(fp);
  return; 
end
...
%        disp(['colortable with ' num2str(colortable.numEntries) ' entries read (originally ' colortable.orig_tab ')']);
        ...
        version = -numEntries;
        if(version~=2)    
          fprintf('%s: ERROR: This version (%d) is not supported.\n',mfilename,version);
        % else
        ...
        for i = 1:numEntriesToRead
            ...
            if (structure < 0)
              fprintf('%s: ERROR: The structure %d is invalid.\n',mfilename,structure);
            end
            if(~isempty(colortable.struct_names{structure}))
              fprintf('%s: ERROR: The structure %d is duplicated.\n',mfilename,structure);
            end
            len = fread(fp, 1, 'int');
...
%        disp(['colortable with ' num2str(colortable.numEntries) ' entries read (originally ' colortable.orig_tab ')']);
  ...
  fprintf('%s: ERROR: The annotation file contains invalid values.\n',mfilename);
end
