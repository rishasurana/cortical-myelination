function [] = timepointT1T2Analysis(subjectFolder, subject, slicerDir4102, scriptLocation, TP1, TP2)
        b = 1;
        ...
        cd([slicerDir4102]);
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/ResampleScalarVolume -s 1,1,1 ' subjectFolder ...
            '/' subject '/' TP1 'fs/mri/orig/m001.nii ' subjectFolder '/' subject ... 
        ...
        NoBrain = brainmask.*(controlT2Align(alignNewVol(BrainT1Aligned)));
                %NoBrain = brainmask.*(controlT2Align(alignNewVol(BrainT1Aligned)));
        ...
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/BRAINSFit --fixedVolume ' subjectFolder '/' subject ...
            '/' TP1 'fs/mri/' 'brain.mgz --movingVolume ' subjectFolder '/' subject ...
            '/' TP1 'fs/mri/orig/' 'SPMT1NoBrain.mgh --outputVolume ' subjectFolder '/' subject ...
            '/' TP1 'fs/mri/orig/'  'SPMTransformedNoBrain.mgh --transformType Rigid ' ...
       ...
       % NoBrainT2 = brainmask.*controlT2Align(controlT2Align(BrainT2Aligned));
        ...
        aparc(notGray) = 0;
        %%% TODO: Consider intensity normalization of both volumes here
        ...
        gunzip([subjectFolder '/' subject '/' TP2 'fs/mri/brainmask.mgz']);
        gunzip([subjectFolder '/' subject '/' TP2 'fs/mri/orig/001.mgz']);
        gunzip([subjectFolder '/' subject '/' TP2 'fs/mri/aparc.a2009s+aseg.mgz']);
        
        
        [BrainMask,  M16, ~, ~] = fs_load_mgh([subjectFolder '/' subject '/' TP2 ... 
            'fs/mri/brainmask']);
                holder{b} = M16;
        b = b + 1;
        cd([slicerDir4102]);
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/ResampleScalarVolume -s 1,1,1 ' subjectFolder ...
            '/' subject '/' TP2 'fs/mri/orig/m001.nii ' subjectFolder '/' subject ... 
            '/' TP2 'fs/mri/orig/SPM001resliced.mgz']);
        %system(['./Slicer --launch lib/Slicer-4.10/cli-modules/ResampleScalarVolume -s 1,1,1 ' subjectFolder ...
        %    '/' subject '/' timepoint 'fs/mri/orig/001.mgz ' subjectFolder '/' subject ... 
        %    '/' timepoint 'fs/mri/orig/001resliced.mgz']);
        gunzip([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPM001resliced.mgz']);
        
        [BrainT1,  M2, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPM001resliced']);
                holder{b} = M2;
        b = b + 1;
        M2(1,4) = M16(1,4);
        M2(2,4) = 256 + M16(2,4);
        ...
        BrainT1Nikhil = MatchVolume2Dim(BrainT1, [256 256 256]);
                holder{b} = M2;
        b = b + 1;
        save_mgh(BrainT1Nikhil, [subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT1Padded.mgh'], M2);
        [BrainT1Aligned,  M3, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/'  ... 
            TP2 'fs/mri/orig/SPMT1Padded.mgh']);
                holder{b} = M3;
        b = b + 1;
        brainmask = logical(BrainMask);
        %NoBrain = brainmask.*controlT2Align(alignNewVol(BrainT1Aligned)); 
        NoBrain = brainmask.*(controlT2Align(alignNewVol(BrainT1Aligned))); 
                holder{b} = M4orig;
        b = b + 1;
        save_mgh(NoBrain, [subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT1NoBrain.mgh'], M1); %%changed M1 to M3 to now M2
        %system(['./Slicer --launch lib/Slicer-4.10/cli-modules/BRAINSFit --fixedVolume ' subjectFolder '/' subject ...
        %    '/' TP1 'fs/mri/orig/' 'SPMTransformedNoBrain.mgh --movingVolume ' subjectFolder '/' subject ...
        %    '/' TP2 'fs/mri/orig/' 'SPMT1NoBrain.mgh --outputVolume ' subjectFolder '/' subject ...
        %    '/' TP2 'fs/mri/orig/'  'SPMT1regist' TP1 'T1.mgh --transformType Rigid,ScaleVersor3D,ScaleSkewVersor3D,Affine,BSpline ' ...
        %    '--samplingPercentage 0.5']);
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/BRAINSFit --fixedVolume ' subjectFolder '/' subject ...
            '/' TP1 'fs/mri/orig/' 'SPMTransformedNoBrain.mgh --movingVolume ' subjectFolder '/' subject ...
            '/' TP2 'fs/mri/orig/' 'SPMT1NoBrain.mgh --outputVolume ' subjectFolder '/' subject ...
            '/' TP2 'fs/mri/orig/'  'SPMT1regist' TP1 'T1.mgh --transformType Rigid,ScaleVersor3D,ScaleSkewVersor3D,Affine,BSpline ' ...
            '--samplingPercentage 0.1']);
        [BrainRegistered,  M4, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/' TP2 'fs/mri/orig/' 'SPMT1regist' TP1 'T1.mgh']);
                holder{b} = M4;
        b = b + 1;
        ...
        cd([slicerDir4102]);
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/ResampleScalarVolume -s 1,1,1 ' subjectFolder '/' ... 
            subject '/' TP2 'fs/mri/mT2.prenorm.nii ' subjectFolder '/' subject '/' TP2 ... 
            'fs/mri/orig/SPMT2Resliced.mgz']);
        gunzip([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT2Resliced.mgz']);
        [BrainT2,  T2M2, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT2Resliced']);
                holder{b} = T2M2;
        b = b + 1;
        T2M2(1,4) = M1(1,4);
        T2M2(2,4) = 256 + M1(2,4);
        ...
        BrainT2 = MatchVolume2Dim(BrainT2, [256 256 256]);
                holder{b} = T2M2;
        b = b + 1;
        save_mgh(BrainT2, [subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT2padded.mgh'], T2M2);
        [BrainT2Aligned,  M3, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT2padded.mgh']);
                holder{b} = M3;
        b = b + 1;
        %NoBrainT2 = brainmask.*controlT2Align(controlT2Align(BrainT2Aligned));
                NoBrainT2 = brainmask.*(BrainT2Aligned);
                ...
                holder{b} = T2M2;
        b = b + 1;
        save_mgh(NoBrainT2, [subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMNoBrainT2.mgh'], M4); %chnanged from M1 to T2M2
        system(['./Slicer --launch lib/Slicer-4.11/cli-modules/BRAINSFit --fixedVolume ' subjectFolder '/' subject ...
            '/' TP1 'fs/mri/orig/' 'SPMTransformedNoBrain.mgh --movingVolume ' subjectFolder '/' subject ...
            '/' TP2 'fs/mri/orig/' 'SPMNoBrainT2.mgh --outputVolume ' subjectFolder '/' subject ...
            '/' TP2 'fs/mri/orig/'  'SPMT2regist' TP1 'NoBrain.mgh --transformType Rigid,ScaleVersor3D,ScaleSkewVersor3D,Affine,BSpline ' ...
            '--samplingPercentage 0.1']);
        %system(['./Slicer --launch lib/Slicer-4.10/cli-modules/BRAINSFit --fixedVolume ' subjectFolder '/' subject ...
        %    '/' TP1 'fs/mri/orig/' 'SPMTransformedNoBrain.mgh --movingVolume ' subjectFolder '/' subject ...
        %    '/' TP2 'fs/mri/orig/' 'SPMNoBrainT2.mgh --outputVolume ' subjectFolder '/' subject ...
        ...
        [BrainRegisteredT2,  M4, ~ , ~ ] = fs_load_mgh([subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPMT2regist' TP1 'NoBrain.mgh']);
                holder{b} = M4;
        b = b + 1;
        %%%%%%%%%% Ratio Volumes Creating
        ...
        ratioVolume2(b2) = 0;
        ratioVolume2 = ratioVolume2.*logical(aparc);
        save_mgh(ratioVolume2, [subjectFolder '/' subject '/' TP2 'fs/mri/orig/SPM' TP2 'to' TP1 'ratioVolume.mgh'], M1);
