...
if ~isempty(roigrouplist) && isfield(roigrouplist,'roi')
  for i=1:nresults
    roigrouplist(i).roicode = roigrouplist(i).roi;
  end;
end;
...
for i=1:nresults
  ...
  if isempty(roigrouplist)
    ...
    results(i).roicode = roigrouplist(i).roicode;
    results(i).roiname = deblank(roigrouplist(i).roiname);
    i_roi = intersect(segroicodes,roigrouplist(i).roicode);
    roi = find(ismember(segvol(:),roigrouplist(i).roicode));
    % if aseg file has 0 voxels for any ROI in roigrouplist, do not exclude
    missing_rois = setdiff(roigrouplist(i).roicode,segroicodes); 
